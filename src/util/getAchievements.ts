...
function getAchievementsFromSpec(contributor: Contributor) {
	...
	for (const groupID in spec) {
		...
		if ("getCount" in group) {
			...
			group.stat === "merged_pulls_by_label" ||
			group.stat === "reviews_by_category"
		) {
			const key =
				group.stat === "merged_pulls_by_label" ? group.label : group.category;
			count = repo
				? contributor[group.stat][repo]?.[key] || 0
				: Object.values(contributor[group.stat]).reduce(
						(sum, repo) => sum + (repo[key] || 0),
						0,
					);
		} else {
