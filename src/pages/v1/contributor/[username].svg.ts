...
const Stat = (
	...
	i: number,
) => `<svg x="13" y="${45 + i * 38}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="14" height="14">
${icons[type]}
</svg>
<text font-weight="bold" font-size="11" x="20" y="${
	71 + i * 38
}" text-anchor="middle">${count}</text>`;

const Achievement = (
	{ achievements }: EnhancedContributor["achievements"][number],
	i: number,
) =>
	`<text x="50" y="${50 + i * 22}">${
		achievements[0].title
	} <tspan font-size="12" fill="#bbb">${
...
export const GET: InferStaticAPIRoute<typeof getStaticPaths> = async ({
	params,
}) => {
